...
class ConLoss(nn.Module):
    ...
    def __init__(self, eps=1e-6, l1_scale=10, bce_scale=5,con_scale = 0.1):
        super(ConLoss, self).__init__()
        self.dice_loss = DiceLoss(eps=eps)
        self.con_scale = con_scale
        self.dice_loss2 = DiceLoss(eps=eps)
        self.dice_loss3 = DiceLoss(eps=eps)
        self.contrast_loss = ContrastLoss(hidden_dim = 32, threshold = 0.3) 
    ...
    def forward(self, pred, batch):
        ...
        con_loss = self.contrast_loss(pred['fea'], pred['binary'], batch['gt'], batch['mask'])

        loss =  bce_loss * 6 + self.con_scale*con_loss
        metrics = dict(bce_loss=bce_loss)

        metrics['con_loss'] = con_loss
        return loss, metrics 
