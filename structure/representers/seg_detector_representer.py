...
class SegDetectorRepresenter(Configurable):
    ...
    def boxes_from_bitmap(self, pred, _bitmap, dest_width, dest_height):
        ...
        for index in range(num_contours):
            ...
            middle_width,middle_height = (points[0] + points[1] + points[2] + points[3])/4
            for k in range(points.shape[0]):
                dis_x =  middle_width - points[k, 0]
                dis_y =  middle_height - points[k, 1]
                poly_recover.append([middle_width - dis_x*10/6, middle_height - dis_y*10/6])
            box = np.array(poly_recover).astype(np.int32)
